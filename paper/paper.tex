...
is permitted by subsidiary recursion.
...
original list.  Our approach does not require switching libraries or
...
functor for lists, paramatrized by the type \verb|A| of elements:
...
We have previously dubbed interfaces for recursion \emph{recursion
universes}~\cite{stump20}.  As in other domains using the term
...
In this paper, our examples use \verb|ListF A| (shown above) to
instantiate the parameter \verb|F|.  \verb|List| is then defined to be
\verb|Subrec|. In general, to use our development to get subsidiary
...
\subsection{Algebras for subsidiary recursion (\texttt{Subrec.v})}

Let us now look more formally at the notion of algebra we introduced
informally above.  The central definitions are in Figure~\ref{fig:algf}.
\verb|KAlg| is the kind for the type-constructor for algebras, as we
see in the definition of \verb|Alg|.  This type-constructor \verb|Alg|
is a fixed-point of the type \verb|AlgF|.  The fixed-point is taken
using \verb|MuAlg|, which implements our retractive-positive recursive
types (Section~\ref{sec:mu}) at kind \verb|KAlg|.

We need a fixed-point here since \verb|Alg| occurs in the definition
...
Let us look further at \verb|AlgF|.  As noted already, each recursion is based
...
operations that follow.  We have a local
...
type \verb|R|.  Finally, we have the subdata structure \verb|d : F R|.
...
\verb|R|, but view those results with type \verb|C| outside
the recursion.  The function \verb|fold| (Figure~\ref{fig:algf}) initiates
top-level folds.  We also have functions \verb|rollAlg| and \verb|unrollAlg|
between \verb|Alg| and its \verb|Algf|-unfolding.  We will
fill in missing definitions (for \verb|Subrec|, \verb|inn|, \verb|fold|, etc.) in Section~\ref{sec:deriv}.
...
(\verb|FmapId| expresses the identity law for functors, needed
for the proof.)
...
Our first example is \verb|span| discussed above.  It does not invoke
subsidiary recursions, but will be used as a subsidiary
recursion in later examples.  Given a predicate
\verb|p : A -> bool|, and a value of type \verb|List A|, we would like
to compute a pair of type \verb|list A * List A|, where the first
component is the maximal prefix whose elements satisfy \verb|p|, and
the second is the remaining suffix.  This is the typing for a
top-level recursion.  More generally, though, given a type
\verb|R : Set| along with a fold function for that type (i.e., of type
\verb|FoldT (Alg (ListF A)) R|), we will map an input list of type
\verb|R| to a pair of type \verb|list A * R|.  The first component of
this pair is going to be built up from scratch, and so cannot have
type \verb|R|; we cannot statically ensure that outer recursions on it
would be legal.  But the second component will always be a subdatum of the
input list, and so can still have type \verb|R|.  The typing is:
\begin{minted}{coq}
...
of the following type \verb|SpanF R| (with \verb|X| implicit for the constructors):
\begin{minted}{coq}
...
\verb|SpanAlg|, whose type is \verb|Alg (ListF A) SpanF|.  We are defining
...
prefix (\verb|l|) and the suffix (\verb|r|).  We also define a version \verb|breakr| 
for subsidiary recursion.
...
The code for \verb|WordsByAlg| is essentially the same as
\verb|wordsBy| in Haskell, which we saw in Section~\ref{sec:intro}.
Recall that we must drop elements that satisfy \verb|p|, and return
the list of sublists between maximal sequences of such elements.  The
algebra pattern-matches on \verb|xs : ListF A R|. In the \verb|Cons|
case, if the head (\verb|hd|) satisfies the predicate, then we drop it
and recurse.  Legality of the recursive call follows by typing, since
\verb|tl : R| has the input type required by
\verb|wordsBy : R -> list (list A)|.  Otherwise, we use \verb|breakr|
to obtain the maximal prefix \verb|w| of \verb|tl| that does not
satisfy \verb|p|, and the remaining suffix \verb|z|.
...
\verb|fo|.  This means that from the invocation of
...
recursion \verb|wordsByr| and top-level recursion \verb|wordsBy|.
...
\verb|b| to include in the output list, and whatever sublist it
...
Given this definition, Coq code for \verb|mapThrough| is shown in Figure~\ref{fig:mapthrough}.  
\verb|MapThroughAlg| is similar to the
Haskell code in Figure~\ref{fig:mapthroughhs}, though when we call \verb|f|, we must
...
theory.  Here we derive a different solution.
...
For a simple example of noncanonicity: suppose we instantiate \verb|F| with
\verb|ListF A| (from Section~\ref{sec:recu}).  Our derivation actually uses a
...
The type \verb|SubrecF Subrec| is definitionally equal to
\begin{minted}{coq}
forall (X : Set -> Set) (FunX : Functor X), Alg X -> X Subrec
\end{minted}
\noindent So \verb|Subrec| is the type of functions which,
...
\verb|FunX|) over \verb|d : Subrec|, we \verb|unroll| \verb|d| and apply that to the algebra (with its
carrier).
...
reason about these?  We turn now briefly to our interface for subsidiary induction in Coq, and
...
algebra and a value of type \verb|C d|.  \verb|AlgFi| and \verb|Algi| are indexed versions
...
well-foundedness then propagates through code.  Our
approach, while less general, does not clutter code with proofs.
