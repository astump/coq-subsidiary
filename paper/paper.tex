...
\verb|Data.List|).  Code is in Figure~\ref{fig:wordsBy}, using
also \verb|break| (\verb|Data.List|). Recall that
\verb|break p| is equivalent to \verb|span (not . p)|: it splits its
input into the maximal prefix whose elements do not satisfy \verb|p|,
and the following suffix.  The first recursive call,
...
will be permitted by subsidiary recursion as derived below.
