...
functions like \verb|wordsBy|, and prove theorems about them.  A nice
...
\subsubsection{Subrec.v}
  ...
  reasons, we just give the example of lists.  The file
...
Section~\ref{sec:subrecimpl}).  This value takes in a carrier
...
\begin{itemize}
\item \verb|Subrec| is passed as the value for the abstract type \verb|R|.
\item the identity function is passed as the value for \verb|reveal : R -> Subrec| (since \verb|R| has
  been instantiated to \verb|Subrec|).
...
\item For the subdata structure of type \verb|F R|, we pass \verb|d : F Subrec| (again, since we
  are instantiating \verb|R| with \verb|Subrec|).
\end{itemize}
...
\subsection{Converting between \texttt{list} and \texttt{List} (\texttt{List.v})}

Figure~\ref{fig:listb} gives the definitions (specified in
Figure~\ref{fig:listf} above) for conversion functions between
\texttt{list} (the list type of Coq's standard library) and our
\texttt{List} type for subsidiary recursion on lists.  To convert a
\verb|list| to a \verb|List|, we use \verb|listFold| to recursively
apply the function \verb|inn| (Figure~\ref{fig:subrecb}) throughout a
\verb|list|.  This \verb|listFold| provides a functorial view of the
usual fold-right function on lists.  To convert a \verb|List| to a
\verb|list|, we use our \verb|fold| function
(Figure~\ref{fig:subrecb}) to fold an algebra over the input
\verb|List|.  This algebra does an \verb|fmap| (for \verb|ListF|) with
\verb|eval : R -> list A|.  The resulting value has type
\verb|ListF (list A)|.  We may then apply \verb|listIn| to produce a
\verb|list A|.  This \verb|listIn| function converts from
\verb|ListF (list A)| to \verb|list A| in the obvious way, converting
\verb|Nil| to the empty list, and \verb|Cons| to an application of
\verb|::|.  These functions were not necessary in implementing
examples of subsidiary recursion above, but they will be needed
for reasoning about them.  We turn to this topic next.
...
\end{verbatim}
\caption{Additional functions for the \texttt{List} type (\texttt{Subrec ListF})}
\label{fig:listb}
\end{figure}
...
constant indexed-functor returning the types shown).  But for the
third, \verb|GuardPres|, we are showing that whenever \verb|spanh|
(see Figure~\ref{fig:span}) returns a suffix \verb|r|, that suffix
satisfies the indexed anchor type.  This enables us to invoke an
outer induction hypothesis on this suffix, when using \verb|span|
subsidiarily.
...
Using these lemmas, we can write a short proof by subsidiary induction of:
